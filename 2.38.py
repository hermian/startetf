...
# %% [markdown]
# 월초리밸런싱을 가정하기 위해 월초로 된 데이터를 구성한다.
...
# %%
start = '1986-1-31' # 최초 매수일

# %%
r_ms.set_date_range(start)
r_ms.display()
...
# %%
r_ms.set_date_range(start)
r_kospi.set_date_range(start)
r_현금.set_date_range(start)
...
# %%
r_daily.set_date_range(start)
r_daily.display()

# %%
r_daily.prices[start:].resample('M').last().to_drawdown_series().describe()
...
r_kospi1.set_date_range(start, e)
r_현금1.set_date_range(start, e)
...
# %%
dd = pd.DataFrame()
dd.index = pd.date_range(start, e, freq='D')
denominator = len(pd.date_range(start, e, freq='D'))/len(pd.date_range(start, e, freq='Y'))
dd['현금'] = pow(1.03, 1/denominator)
dd['현금'] = dd['현금'].shift(1, fill_value=1.0)
dd['현금'] = dd['현금'].cumprod()
#%%
